...
fn main() {
    ...
    for (i, road) in good_roads.iter().take(20).enumerate() {
        ...
        println!("   Length: {:.1}m ({:.2}km)", road.length, road.length / 1000.0);
        println!("   Lanes: {}", road.lane_count);
        
        if let Some(info) = validator.get_road_info(&road.id) {
            let driving_lanes = info.lanes.iter()
    ...
    if let Some(best) = good_roads.first() {
        ...
        println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        println!("  Length: {:.1}m ({:.2}km)", best.length, best.length / 1000.0);
        ...
        match validator.suggest_spawn_points(&best.id, 5) {
            Ok(points) => {
                ...
                for (i, point) in points.iter().enumerate() {
                    println!("    {}. lane={}, s={:.1}m ({})", 
                             i + 1, point.lane_id, point.s, point.description);
                }
            }
